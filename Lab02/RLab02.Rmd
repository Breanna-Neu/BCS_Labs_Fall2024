...
```{r summary}
dat.76 %>% summarise(mean(Age, na.rm = T), median(Age, na.rm = T))
```
...
```{r}
dat.76.clean %>% summarise(mean(Age))
...
```{r}
CBdata.Clean %>% filter(Year < 1978) %>% summarise(mean(Age))
CBdata.Clean %>% filter(Year > 1977) %>% summarise(mean(Age))
```
