...
1) First load the data, which is saved as a .RData file called `CBdata1_10.RData`. This is the first ten years' worth of Cherry Blossom Race data. Pull out the data associated with 1976 and store it as a data frame called `dat.76`. Remove the column `Pis/Tis` using a `dplyr` function. 


```{r import_data}
library(dplyr) 
load("CBdata1_10.RData") #Loading the cherry blossom data. (this is an example of a properly commented line of code)

dat.76 <- data.frame(CBdata.1_10[4]) #Extracting the data for 1976
dat.76 <- dat.76 %>% select(-PiS.TiS) #Removing the column Pis/Tis
```
...
```{r summary}
dat.76 %>% summarise(mean(Age, na.rm = T), median(Age, na.rm = T)) #Calculating the mean and median ages in 1976
```
...
dat.76.clean <- dat.76 %>% filter(!is.na(Age)) #Removing the rows with missing age values
```
...
```{r combine_dat}
CBdata.Clean <- CBdata.1_10 %>% #Combining all the data
  bind_rows(list()) %>%  #Binding the rows
  select(-'PiS/TiS') %>% #Removing the column Pis/Tis
  filter(!is.na(Age)) #Removing the rows with missing age values
```
...
```{r}
dat.76.clean %>% summarise(mean(Age)) #Calculating the average age in 1976
CBdata.Clean %>% summarise(mean(Age)) #Calculating the average age over the first 10 years
```

From Lab 1: Average Age in 1976 32.09356, Average Age over first 10 years 33.26157.
From Lab 2: Average Age in 1976 32.09356, Average Age over first 10 years 33.26157.
They do match.
...
```{r}
CBdata.Clean %>% filter(Year < 1978) %>% summarise(mean(Age)) #Calculating the average age over the first 5 years
CBdata.Clean %>% filter(Year > 1977) %>% summarise(mean(Age)) #Calculating the average age over the next 5 years
```

From Lab 1: Average Age 1973-1977 31.11927, Average Age 1978-1982 33.69101.
From Lab 2: Average Age 1973-1977 31.11927, Average Age 1978-1982 33.69101.
They do match.
...
```{r}
library(readr)
weatherdata <- read_csv("weatherdat.csv")
head(weatherdata)
```
...
```{r}
temp<-left_join(CBdata.Clean, dates, by = c("Year" = "year"))
sample_n(temp, 10)
```
```{r}
sample_n(weatherdata, 3)
sample_n(temp, 3)
```
...
```{r}
CBdata.Clean <- left_join(temp, select(weatherdata, DATE, PRCP, TMIN), by = c("date" = "DATE"))
sample_n(CBdata.Clean, 10)
...
IPS_sampledata <- IPS_sampledata %>%
  mutate(
    # Convert the time variable from its original type to numeric
    # This ensures that time can be further transformed and analyzed
    time = as.numeric(time),
    
    # Convert scanMac to a character type since it represents MAC addresses (not numbers)
    scanMac = as.character(scanMac),
    
    # Convert the physical coordinates (posX, posY, posZ) to numeric for mathematical operations
    posX = as.numeric(posX),
    posY = as.numeric(posY),
    posZ = as.numeric(posZ),
    
    # Convert orientation to numeric as it represents degrees (can be used in calculations)
    orientation = as.numeric(orientation),
    
    # Convert mac (access point MAC address) to a character type for categorization
    mac = as.character(mac),
    
    # Convert signal strength to numeric for analysis (e.g., calculating averages)
    signal = as.numeric(signal),
    
    # Convert channel frequency to numeric for proper representation
    channel = as.numeric(channel),
    
    # Convert type to integer since it is a categorical variable (1 or 3)
...
IPS_sampledata <- IPS_sampledata %>%
  filter(
    # Keep only rows where the type column equals 3 (indicating access points)
    # This removes other types of devices (e.g., adhoc devices with type == 1)
    type == 3
  )
```
...
IPS_sampledata <- IPS_sampledata %>%
  select(
    # Exclude scanMac as it represents the handheld device (not relevant for analysis)
    # Exclude type as it is now constant (all rows are type == 3 after filtering)
    -scanMac, 
    -type
  )
```
...
```{r}
# Transform the `time` variable from milliseconds to seconds and then convert it to POSIXct format
IPS_sampledata <- IPS_sampledata %>%
  mutate(
    # Divide time by 1000 to convert from milliseconds to seconds
    # Use as.POSIXct to convert the numeric time into a readable datetime format
    # The 'origin' specifies the epoch (1970-01-01), and 'tz' sets the time zone to UTC
...
```{r}
library(tidyr) #loading package tidyr

IPS_sampledata_wide <- IPS_sampledata %>%
  pivot_wider(
    names_from = mac, #creating variables with names of each access point    
    values_from = signal #the values in the new variables are the signal strengths for that access point
  )
glimpse(IPS_sampledata_wide) #checking work
```
...
```{r}
IPS_sampledata %>%
  group_by(mac) %>% #grouping by access point
  tally() %>% #tallying the number of observations for all access points
  arrange(desc(n)) #displaying the counts in descending order
```
...
IPS_sampledata <- IPS_sampledata %>%
  group_by(mac) %>% #grouping by access point
  filter(n() >= 10) %>% #filtering out access points that have less than 10 observations
...
```{r}
IPS_sampledata <- IPS_sampledata %>%
  #creating new variable for the correct orientation values
  mutate(rec_orient = case_when(
    orientation >= 337.5 | orientation < 22.5 ~ 0, #setting value to 0 if original was greater/equal to 337.5 or less than 22.5
    orientation >= 22.5 & orientation < 67.5 ~ 45,#setting value to 45 if original was greater/equal to 22.5 or less than 67.5
    orientation >= 67.5 & orientation < 112.5 ~ 90,#setting value to 90 if original was greater/equal to 67.5 or less than 112.5
    orientation >= 112.5 & orientation < 157.5 ~ 135,#setting value to 135 if original was greater/equal to 112.5 or less than 157.5
    orientation >= 157.5 & orientation < 202.5 ~ 180,#setting value to 180 if original was greater/equal to 157.5 or less than 202.5
    orientation >= 202.5 & orientation < 247.5 ~ 225,#setting value to 225 if original was greater/equal to 202.5 or less than 247.5
    orientation >= 247.5 & orientation < 292.5 ~ 270,#setting value to 270 if original was greater/equal to 247.5 or less than 292.5
    orientation >= 292.5 & orientation < 337.5 ~ 315#setting value to 315 if original was greater/equal to 292.5 or less than 337.5
  ))
```
...
```{r}
signal_summary <- IPS_sampledata %>%
  group_by(posX, posY, posZ, rec_orient, mac) %>% #grouping by location, orientation, and access point id
  summarise(
    mean_signal = mean(signal, na.rm = TRUE), #average signal strength for each combination
    sd_signal = sd(signal, na.rm = TRUE), #standard deviation of signal strength for each combination
    .groups = 'drop' #ungrouping data
  )

head(signal_summary) #checking work
```